...
exports.userlikes_getbyUser = async (req, res) => {
    const data = await Userlikes.find({ "user": req.query.user }).populate("movie");
    res.send(data);
};

exports.userlikes_deleteMovieID = async (req, res) => {
    console.log(req.query.id);
    await Userlikes.deleteOne({ "_id": req.query.id});
    res.send({ message: 'Userlikes deleted' });
};